(function($) {
						...
						switch(parse_variable) {
							...
							case 'ecommerce_field_price' :
								...
								if(parse_variable == 'field') {
									...
									delimiter = (typeof(variable_attribute_array[1]) !== 'undefined') ? variable_attribute_array[1] : ',';
	...
	$.WS_Form.prototype.get_data_grid_column = function(parsed_variable, field, column) {
		...
		var return_column_index = parseInt(column);
		...
		for(var group_index in groups) {
			...
			for(var row_index in rows) {
				...
				if(typeof(data[value_column_index]) === 'undefined') { continue; }
				if(typeof(data[return_column_index]) === 'undefined') { continue; }
				...
				if(
					...
					(typeof(data[label_column_index]) !== 'undefined')
				) {
				...
				if(
					...
					(typeof(data[parse_variable_column_index]) !== 'undefined')
				) {
				...
				for(var data_index in data) {
					...
					if(!data.hasOwnProperty(data_index)) { continue; }

					data[data_index] = this.mask_parse(data[data_index], mask_values_row);

					if(typeof(data[data_index]) === 'number') { data[data_index] = data[data_index].toString(); }
				}
				...
				for(var parsed_variable_index in parsed_variable) {
					...
					if(parsed_variable_single === data[value_column_index].toString()) {
